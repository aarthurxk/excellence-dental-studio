...
import {
  ...
  AlertTriangle, Clock, PlugZap, Activity,
} from "lucide-react";
...
function ConexaoTab() {
  ...
  async function handleReconnect() {
    ...
    try { await evoProxy("connect"); toast.success("Solicitação de reconexão enviada"); setTimeout(checkStatus, 3000); } catch { toast.error("Erro ao reconectar"); }
...
function DashboardTab() {
  ...
  const handleReconnect = async () => {
    setReconnecting(true);
    try { const res = await evoProxy<{ pairingCode?: string; code?: string }>("connect"); toast.success(`Reconectando... Código: ${res.pairingCode || res.code || "Enviado"}`); qc.invalidateQueries({ queryKey: ["evo-connection"] }); } catch (e: any) { toast.error("Erro ao reconectar: " + e.message); } finally { setReconnecting(false); }
  };
      ...
      {connLoading ? (
        ...
        <Card className={isConnected ? "border-green-500/50" : "border-red-500/50"}>
          <CardContent className="flex items-center justify-between p-6">
            ...
            {isConnected && <Badge variant="outline" className="text-green-600 border-green-500">● Online</Badge>}
          </CardContent>
        </Card>
      )}
